...
export function FinancialChat() {
  ...
  // Automatically select the first thread if one exists and none is selected
  useEffect(() => {
    if (!threadId && threads.length > 0) {
      selectThread(threads[0].id);
    }
  }, [threads, threadId, selectThread]);
    ...
    <div className="flex flex-col h-[calc(80vh-120px)] rounded-md overflow-hidden border border-gray-200">
      <div className="bg-white shadow-sm p-4 rounded-t-md flex justify-between items-center">
        ...
        <button onClick={handleToggleThreads} className="text-sm text-primary hover:text-primary/80 flex items-center gap-1">
          ...
          <span>{isThreadsOpen ? "Hide History" : "Show History"}</span>
        </button>
      </div>
